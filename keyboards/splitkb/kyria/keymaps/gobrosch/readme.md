...
This keymap is based on the Kyria Default Keymap below. The following changes were made before Kyria rev.2 was added to the official QMK repo (see also log.txt): 

    add CapsLock to SYM layer;
    correct right parentheses
    add umlaute to Colemak base layer
    disable RGB animations to save space;
    enable AutoShift
    change RGB lighting depending on layer
